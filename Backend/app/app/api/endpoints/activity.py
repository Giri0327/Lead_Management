...
async def create_activity(activity:Lead_Activity,current_user = Depends(role_required([1,2])),db:session=Depends(get_db)):
...
async def get_activity(lead_id:Optional[int]=None, current_user = Depends(role_required([1,2])),db:session=Depends(get_db)):
...
async def activity_Details(lead_id:int=None,current_user = Depends(role_required([1,2])),db:session=Depends(get_db)):
...
async def view_Files(activity_id:Optional[int]=None,current_user = Depends(role_required([1,2])),db:session=Depends(get_db)):
    creator = Files(activity_id,None,None, db)
