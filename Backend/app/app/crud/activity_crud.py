...
class Activity:
    ...
    def view_activity(self, lead_id, current_user):
        ...
        query = (
            self.db.query(
                Lead_Activity.Notes, Lead_Activity.Scheduled_On, User.Username
...
class Files:
    ...
    def add_file(self, current_user, file: UploadFile = File(...)):
        ...
        activity = (
            self.db.query(Lead_Activity)
            .filter(Lead_Activity.Activity_ID == self.activity_id)
            .first()
        )
        ...
        result = cloudinary.uploader.upload(
            file.file, public_id=filename, format=extension
        )
        ...
        short_url = url.replace(
            "https://res.cloudinary.com/dedavidqu/image/upload/", "CLOUDINARY/"
        )

        print(short_url)

        file_data = Activity_file(Activity_ID=self.activity_id, File_url=short_url)

        self.db.add(file_data)
        self.db.commit()
        self.db.refresh(file_data)

        return {"message": "File Added", "file_Name": filename, "file_URL": url}
