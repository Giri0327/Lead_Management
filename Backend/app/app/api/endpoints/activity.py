...
def create_activity(activity:Lead_Activity,token = Depends(get_current_user),db:session=Depends(get_db)):
    creator = Activity(activity, db)
    new_lead = creator.add_activity()
    return new_lead

@router.get("/view_Activity")
def get_activity(lead_id:int,token = Depends(get_current_user),db:session=Depends(get_db)):
...
def activity_Details(lead_id:int,current_user = Depends(get_current_user),db:session=Depends(get_db)):
    creator = Details(lead_id, db)
    view_details = creator.show_details(lead_id)
    return view_details

@router.post("/add_File")
def create_File(activity_id:int,activity:File_Activity,token = Depends(get_current_user),db:session=Depends(get_db)):
    creator = Files(activity_id,None,activity,db)
    new_lead = creator.add_file()
    return new_lead

@router.get("/view_Files")
def view_Files(activity_id:int,token = Depends(get_current_user),db:session=Depends(get_db)):
...
@router.get("/view_all_lead_notes")
def view_all_notes(user_id: int,token = Depends(get_current_user), db: session = Depends(get_db)):
