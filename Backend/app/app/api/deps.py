...
def get_current_user(
    ...
    try:
        ...
        sessionn = (
            db.query(Token)
            .filter(Token.Token == actual_token, Token.update_At == None)
        ...
        if datetime.utcnow() - sessionn.Token_Expiry > INACTIVITY_LIMIT:
            ...
            sessionn.update_At = datetime.utcnow()
            sessionn.Token = None
            db.commit()
        ...
        sessionn.Token_Expiry = datetime.utcnow()
        db.commit()
        ...
        sessionn = db.query(Token).filter(Token.Token == actual_token).first()
