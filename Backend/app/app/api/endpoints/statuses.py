...
def status_create(user:Status,current_user = Depends(role_required([1])),db:session=Depends(get_db)):
    return create_status(user,db)

@router.get("/view_Status")
def view_status(current_user = Depends(role_required([1,2])),db:session=Depends(get_db)):
    return view_all_status(db)

@router.put("/update_Status")
def update_status_name(status_id:int,user:Status,current_user = Depends(role_required([1])),db:session=Depends(get_db)):
    return update_status(status_id,user,db)

@router.delete("/status_Delete")
def status_delete(status_id:int,current_user = Depends(role_required([1])),db:session=Depends(get_db)):
