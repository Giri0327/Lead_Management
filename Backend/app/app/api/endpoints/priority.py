...
async def priority_create(user:Priority_Schema,current_user = Depends(role_required([1])),db:session=Depends(get_db)):
...
async def update_priority_name(priority_id:int,user:Priority_Schema,current_user = Depends(role_required([1])),db:session=Depends(get_db)):
